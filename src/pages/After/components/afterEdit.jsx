...
import { edit_after_after } from "@/services/myapi";
...
const Edit = (props) => {
  ...
  useEffect(async () => {
    ...
    if (editOrder !== undefined) {
      const response = await edit_after_after(editOrder, editSKU);
      ...
      if (response.length == 0) {
        ...
        initial_dic = {
          ...
          订单号: response[0].订单号,
          SKU: response[0].SKU,
          订单状态: response[0].订单状态,
          Refund: response[0].Refund,
          备注: response[0].备注,
          序号: response[0].序号,
          是否上传图片: response[0].是否上传图片,
        };
        // 对顾客反馈和客服操作进行处理
        response[0].顾客反馈 = response[0].顾客反馈.split("&");
        response[0].客服操作 = response[0].客服操作.split("&");
        const temp_fankui = new Array();
        const temp_yuanying = new Array();
        const temp_renew = new Array();
        for (const key in response[0].顾客反馈) {
          if (Object.hasOwnProperty.call(response[0].顾客反馈, key)) {
            let element = response[0].顾客反馈[key];
            element = element.split('$');
            const temp_first = element[0].split('-');
            response[0].顾客反馈[key] = [temp_first[0] + '-0', element[0]];
            if ((element.length > 1)) {
        ...
        for (const key in response[0].客服操作) {
          if (Object.hasOwnProperty.call(response[0].客服操作, key)) {
            let element = response[0].客服操作[key];
            element = element.split('$');
            const temp_first = element[0].split('-');
            response[0].客服操作[key] = [temp_first[0] + '-0', element[0]];
            // 数量更换
            if (element.length > 1) {
              ...
              if ((temp_element[0] == '2-1-0') || (temp_element[0] == '2-2-0') || (temp_element[0] == '2-3-0')
                ...
                response[0].客服操作[key] = [temp_first[0] + '-0', temp_element[0].replace(new RegExp('#', ("gm")), ',')];
        ...
        initial_dic.客服操作 = response[0].客服操作
        initial_dic.顾客反馈 = response[0].顾客反馈
